		...
		int update)
{
	...
	case V4L2_PIX_FMT_RGB24:
		...
		min_numlines = height;
	case V4L2_PIX_FMT_RGB32:
		min_hpitch = (inst->win.w.width + inst->win.w.left) * 4;
		min_numlines = height;
	default:
		min_hpitch = (inst->win.w.width + inst->win.w.left) * 4;
		min_numlines = height;
	...
	ti81xxvin_dbg(2, debug, "pixfmt->pixelformat(%x), V4L2_PIX_FMT_RGB32(%x)\n",
		pixfmt->pixelformat, V4L2_PIX_FMT_RGB32);
	...
	if (update) {
		...
		buf_obj->fmt.fmt.pix.height = pixfmt->height;
		buf_obj->fmt.fmt.pix.bytesperline = hpitch;
		buf_obj->fmt.fmt.pix.sizeimage = pixfmt->sizeimage;
	...
	ti81xxvin_dbg(2, debug, "pixfmt->sizeimage(%d), pixfmt->width(%d), pixfmt->height(%d)\n",
		pixfmt->sizeimage, pixfmt->width, pixfmt->height);
		...
		void *priv, struct v4l2_dv_preset *qpreset)
{
	...
	//mbus_framefmt.code = V4L2_MBUS_FMT_YUYV8_1X16;
	...
	inst->video.cur_dv_preset.preset = V4L2_DV_1080P30;
	inst->video.cur_std_id = V4L2_STD_UNKNOWN;
	inst->video.cur_dv_preset = *qpreset;
...
static int vidioc_querystd(struct file *file, void *priv, v4l2_std_id *norm)
{
	...
	inst->video.cur_dv_preset.preset = V4L2_DV_576P50;
	inst->video.cur_std_id = V4L2_STD_625_50;
		...
		void *priv, struct v4l2_dv_preset *dv_preset)
{
	...
	dv_preset->preset = V4L2_DV_1080P30;
	inst->video.cur_dv_preset.preset = V4L2_DV_1080P30;
	inst->video.cur_std_id = V4L2_STD_UNKNOWN;
	inst->video.cur_dv_preset = *dv_preset;
	/* Get the information about the standard */
